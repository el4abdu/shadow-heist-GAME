...
import { subscribeToRoom, Room, Player, GameState, sendMessage, subscribeToChat, Message } from '@/lib/firebase-services';
...
export default function RoomPage() {
  ...
  }, [isLoaded, isSignedIn, user, roomCode, router]);
  
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!messageInput.trim() || !user || !currentPlayer) return;
              ...
              <div className="space-y-3">
                {room.players.map((player) => (
                  ...
                  >
                    ...
                    {room.gameState.status === 'playing' && currentPlayer?.role && (
                      <div className="text-xs">
                        {/* Show role only to self or when game is over */}
            ...
            {room.gameState.status === 'waiting' ? (
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700 p-6 flex flex-col items-center justify-center min-h-[400px]">
                ...
                {currentPlayer?.isHost ? (
                  ...
                  <Button 
                    className="w-48 py-6 bg-blue-600 hover:bg-blue-700"
                    onClick={() => {
                      // Toggle ready state
                      console.log("Ready toggle would go here");
                    }}
                  >
